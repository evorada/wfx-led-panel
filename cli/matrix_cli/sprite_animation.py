...
def load_frames(dir: str = "resources/knight/idle") -> List[ImageFile]:
    ...
    gif_files = glob.glob(os.path.join(dir, "*.gif"))
    frame_files = sorted(png_files + gif_files)
    
    if not frame_files:
        raise ValueError(f"No frames found in {dir}")
...
def run_animation(matrix: MatrixDisplay, frame_delay: float = 0.1, cycles: int = 5):
    ...
    try:
        ...
        for i, frame in enumerate(frames):
            if i >= 12:  # Limit to 12 sprites
                break
            success, msg = matrix.set_sprite(i, center_x, center_y, frame.width, frame.height, frame.tobytes())
